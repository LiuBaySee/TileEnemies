...
namespace TileEnemies
{
    public partial class Form1 : Form
    {
        ...
        Rectangle[] selectedTiles = new Rectangle[5]; //The blue selection indicators
        ...
        private void Form1_MouseClick(object sender, MouseEventArgs e)//directs where units go and selects units
        {
            if (tileCheck[moveableTile[0]] == false && tileCheck[moveableTile[1]] == false && tileCheck[moveableTile[2]] == false && tileCheck[moveableTile[3]] == false)//if mouse is not on any of the moveableTiles...
            {
                ...
                for (int i = 0; i < units.Length; i++)
                {
                    ...
                    if ((units[i].Left <= cursorLocation.X && cursorLocation.X <= units[i].Right) && (units[i].Top <= cursorLocation.Y && cursorLocation.Y <= units[i].Bottom))
                    {
                        ...
                        if (selected[unitCheck] == true)
                        {
                            selected[unitCheck] = false;
                            selected[i] = true;
                        }
            ...
            moving = false;            //When the User clicks anywhere, moving will be false
            for (int i = 0; i < selected.Length; i++)//goes through each selected tile when the user selects a unit...
            {
                for (int j = 0; j < tiles.Length; j++)//and goes through each tile...
                {
                    //The tile its sitting on
                    if (selected[i] == true && units[i].IntersectsWith(tiles[j]))//check if selected unit is on a tile, which it is, this is just ment to single out that tile
                    {
                        ...
                        moving = true; //Allows the moving animation
                    }
                    ...
                    if (selected[i] == true && tiles[currentTile].Left == tiles[j].Right && tiles[currentTile].Top == tiles[j].Top)
                    {
                    ...
                    if (selected[i] == true && tiles[currentTile].Right == tiles[j].Left && tiles[currentTile].Top == tiles[j].Top)
                    {
                    ...
                    if (selected[i] == true && tiles[currentTile].Bottom == tiles[j].Top && tiles[currentTile].Right == tiles[j].Right)
                    {
                    ...
                    if (selected[i] == true && tiles[currentTile].Top == tiles[j].Bottom && tiles[currentTile].Right == tiles[j].Right)
                    {
        ...
        protected override void OnPaint(PaintEventArgs e)//Graphics
        {
            ...
            ////change the colour of tile to translucent blue...  
            ...
            for (int i = 0; i < units.Length; i++)
            {
                ...
                {
                    //allows the selected animation
                    if (moving == true)
                    {
                        //selectable locations
                        selectedTiles[0] = new Rectangle(units[i].X-10, units[i].Y-10, 100, 100);
                        selectedTiles[1] = new Rectangle(units[i].X+90, units[i].Y - 10, 100, 100);
                        selectedTiles[2] = new Rectangle(units[i].X-110, units[i].Y - 10, 100, 100);
                        selectedTiles[3] = new Rectangle(units[i].X-10, units[i].Y+90, 100, 100);
                        selectedTiles[4] = new Rectangle(units[i].X - 10, units[i].Y-110, 100, 100);

                        //draws them all
                        for (int j = 0; j < selectedTiles.Length; j++)
                        {
                            e.Graphics.FillRectangle(transBlue, selectedTiles[j]);
                        }
